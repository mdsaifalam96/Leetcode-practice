...
class Solution:
    ...
    def get_grid_details(self, x, y):
        grid_row = x - (x % GRID_DIM)
        grid_col = y - (y % GRID_DIM)
        return grid_row, grid_col

    def solveSudokuHelper(self, board, row_elements, col_elements, grid_elements):

        for i in range(BOARD_DIM):
            for k in range(BOARD_DIM):
                if board[i][k] == ".":
                    grid_row, grid_col = self.get_grid_details(i, k)
                    ...
                    for m in range(1, BOARD_DIM + 1):
                        if str(m) not in row_elements[i] and \
                            ...
                            if self.solveSudokuHelper(board, row_elements, col_elements, grid_elements):
                                return True
                            else:
                                board[i][k] = "."
    ...
    def solveSudoku(self, board):
        ...
        self.solveSudokuHelper(board, row_elements,
...
def main():
    ...
    solution.solveSudoku(sudoku_board_1)
